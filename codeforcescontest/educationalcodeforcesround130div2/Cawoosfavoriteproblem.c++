...
void solution()
{
    ll n;cin>>n;
    string s,t;cin>>s>>t;
    if(count(all(s),'a')>count(all(t),'a') or count(all(s),'c')>count(all(t),'c') or count(all(s),'b')>count(all(t),'b') or count(all(s),'a')<count(all(t),'a') or count(all(s),'c')<count(all(t),'c') or count(all(s),'b')<count(all(t),'b'))
    {
        cout<<"NO"<<endl;
        return;
    }
    ...
    for(int i=0;i<n;i++)
    {
        if((s[i]==t[i] and (s[i]=='b' or (ab==0 and bc==0))) or (s[i]==t[i] and ab>0 and s[i]=='a') or (s[i]==t[i] and bc>0 and s[i]=='c'))
            continue;
        else if(s[i]=='a' and t[i]=='b' and bc==0)
        {
            ab++;
        }
        else if(s[i]=='b' and t[i]=='a' and ab>0)
        {
            ab--;
        }
        else if(s[i]=='b' and t[i]=='c' and ab==0)
        {
            bc++;
        }
        else if(s[i]=='c' and t[i]=='b' and bc>0)
        {
            bc--;
        }
        else
