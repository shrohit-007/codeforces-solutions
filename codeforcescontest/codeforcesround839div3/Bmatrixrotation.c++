...
void printpermutation(vector<int>v,int i)
{
    if(i==v.size())
    {
        debug(v);
    ...
    for(int cur=i;cur<v.size();cur++)
    {
        swap(v[cur],v[i]);
        printpermutation(v,i+1);
        swap(v[cur],v[i]);
    }
}
void solution()
{
    ...
    if((a<b and c<d and b<d and a<c) or (c<a and d<b and a<b and c<d) or (d<c and b<a and c<a and d<b) or (b<d and a<c and d<c and b<a))
    {
        cout<<"YES"<<endl;
        return;
    }cout<<"NO"<<endl;
