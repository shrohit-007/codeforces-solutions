...
void solution()
{
    int c3,v3,d3,DHRASHTI________________________________;
    int faltu=0,faltu1=0;
    ...
    while(t--)
    {
        ...
        for(int i=2;i<=m;i++)
        {
            if(pos[arr[i-1]]>=pos[arr[i]] or pos[arr[i-1]]+d<pos[arr[i]])
            {
                ans=0;
                break;
            }
            int dis=pos[arr[i]]-pos[arr[i-1]];
            int dis2=d+1-dis;
            int check=n-pos[arr[i]]+pos[arr[i-1]]-1;
            //cout<<dis<<" "<<dis2<<" "<<check<<endl;
            if(dis2<=check)
            {
                dis=min(dis,dis2);
                ans=min(ans,dis);
