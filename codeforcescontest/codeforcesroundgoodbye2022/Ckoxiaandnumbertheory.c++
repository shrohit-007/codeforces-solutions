...
void solution()
{
    ...
    {
        cout<<"NO"<<endl;
        // if any element is occuring twice or more then the 
        // gcd(a+x,a+x) will always be >1 then there ie trivial no
        return;
    }
    // otherwise 
    // suppose there are 4 number 1 2 3 4 
    // two are odd and two are even means any number we will always have 2 even number 
    // means gcd (even,even)>=2 then in this case the answer will no 
    // general case

    // we know gcd(a+x,b+x)>=2 then answer is no 
    // if suppose gcd(a+x,b+x)==2 then (a+x)%2==0 and (b+x)%2==0
    // we are getting 2  0's after taking % of the final numbers
    // now before adding x they would have been something different 
    // we know remainder of any number %2 will be either 1 or 0
    //and we are adding same x to both of them then either the x will be even or x will be odd
    // means that a%2==1 and b%2==1 or (a%b==0 or b%2==0)
    // now intution is that if we have remaiders %2 =(1,1,0,0)
    // in this array two are 0 and two are 1 now if we will add any number to it 
    // which will give (either 0 or 1 after % by 2) then we will have atleast 2 numers
    // which will be divisible by 2 
    // for number 3 if the array elements after % by 3 are (1,1,2,2,0,0)
    // then in the above array the answer will always be no after adding any number to it 
    // because any nubmer %3 will give a integer ranging from (0 to 2) which will make 
    // 2 nubmers (mod by 3) ==0 and gcd will be 3 for those 2 number and ans will be no 

    // hence we will check for every number
