...
{   
    ...
    else
    {
        ...
        for(int i=1;i<n-1;i+=2)
        {
            if(i<n-1 and v[i]<=v[i-1] or v[i]<=v[i+1])
            {
                tmp1+=max(v[i-1],v[i+1])-v[i]+1;
            }
            if(v[i+1]<=v[i] or v[i+1]<=v[i+2])
            {
                ans2-=(max(v[i+2],v[i])-v[i+1]+1);
            }
            //cout<<tmp1<<" "<<ans2<<endl;
            ans=min(tmp1+ans2,ans);
        }
        for(int i=n-2;i>0;i-=2)
        {
            if(i>0 and v[i]<=v[i-1] or v[i]<=v[i+1])
            {
                tmp2+=max(v[i-1],v[i+1])-v[i]+1;
            }
            if(v[i-1]<=v[i] or v[i-1]<=v[i-2])
            {
                ans1-=(max(v[i],v[i-2])-v[i-1]+1);
            }
            ans=min(tmp2+ans1,ans);
        }
    }
    cout<<ans<<endl;
}
