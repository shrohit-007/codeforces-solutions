...
int sum[200003][3];
int mincost(int n,int i,vector<vector<int>>&arr,int p)
{
    if(i==n)
    {
        return 0;
    ...
    if(p==0)
    {
        return sum[i][p]=max(arr[i][1]+mincost(n,i+1,arr,1),arr[i][2]+mincost(n,i+1,arr,2));
    }
    else if(p==1)
    {
        return sum[i][p]= max(arr[i][0]+mincost(n,i+1,arr,0),arr[i][2]+mincost(n,i+1,arr,2));
    }
    else
    {
        return sum[i][p]=max(arr[i][1]+mincost(n,i+1,arr,1),arr[i][0]+mincost(n,i+1,arr,0));
    }
}
int dp[2000005];
int coinchange(int amount,vector<int>&coins)
{
    ...
    {
        if(amount-coin>=0)
        mn=min(mn+0LL,coinchange(amount-coin,coins)+1LL);
    }
    return dp[amount]=mn;
}
void solution()
