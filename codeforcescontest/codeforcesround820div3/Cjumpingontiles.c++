...
void minimummoves(vector<int>&dp)
{
    dp[1]=0;
...
void stringpartition(string s,int n,int i,string st)
{
    if(i==n)
    {
        return;
...
void solution()
{
    ...
    int d=abs(m[s[0]]-m[s[n-1]]);
    int ans=0;
    int x=min(m[s[0]],m[s[n-1]]);
    int y=max(m[s[0]],m[s[n-1]]);
    for(int i=0;i<n;i++)
    {
        if(m[s[i]]>=x and m[s[i]]<=y)
        {
            m1[s[i]].pb(i+1);
            ans++;
        }
